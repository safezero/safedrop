...
const SafedropServer = module.exports = defunction(['number', 'string'], '*', function SafedropServer(difficulty, dbPath) {
  ...
  this.db = new LevelWrapper(dbPath)
})

SafedropServer.prototype.listen = defunction(['number'], '=>SafedropServer', function listen(port) {
  return new Promise((resolve, reject) => {
    this.port = port
    this.httpServer = http.createServer()
    this.httpServer.listen(port, () => {
      this.webSocketSafedropServer = new WebSocket.SafedropServer({ server: this.httpServer })
      this.webSocketSafedropServer.on('connection', (ws) => {
        ...
        ws.on('message', (directiveEncodingBuffer) => {
          ...
          switch (commandPojo.branch) {
            case 'addDrop':
              return this.db.addToList(commandPojo.value.identityPublicKey, commandPojo.value.drop).then(() => {
                ws.send(getResponseEncoding(directivePojo.value.nonce, 0, null))
              })
              break;
            case 'getDrops':
              return this.db.getList(commandPojo.value).then((drops) => {
