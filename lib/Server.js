...
const Server = module.exports = defunction(['number'], '*', function Server(difficulty) {
  ...
  this.blurbsDb = new LevelWrapper('./db/blurbs')
})
      ...
      this.webSocketServer.on('connection', (ws) => {
        ...
        ws.on('message', (directiveEncodingBuffer) => {
          ...
          switch (commandPojo.branch) {
            case 'addBlurb':
              return this.blurbsDb.addToList(commandPojo.value.identityPublicKey, commandPojo.value.blurb).then(() => {
                ws.send(getResponseEncoding(directivePojo.value.nonce, 0, null))
              })
              break;
            case 'getBlurbs':
              return this.blurbsDb.getList(commandPojo.value).then((blurbs) => {
                const blurbsEncoding = itemsTemplate.encode(blurbs)
                ws.send(getResponseEncoding(directivePojo.value.nonce, 0, blurbsEncoding))
              })
              break;
            default:
