...
Client.prototype.addBlurb = defunction(['Persona', 'Uint8Array'], '=>Uint8Array', function addBlurb(persona, blurb) {
...
Client.prototype.getBlurbs = defunction([], '=>[]Uint8Array', function getBlurbs() {
  const commandEncoding = commandTemplate.encode({
    branch: 'getBlurbs',
    value: this.identityKeypair.publicKey
  })
  return this.sendCommandEncoding(this.gateway, commandEncoding).then((blurbsEncoding) => {
    return itemsTemplate.decode(blurbsEncoding)
  })
})
...
Client.prototype.addPlaintext = defunction(['Persona', 'Uint8Array'], 'Promise', function addPlaintext(persona, plaintext) {
  ...
  return this.addBlurb(persona, cryptix.getEncoding()).then(() => {
    ...
    return this.addBlurb(this.persona, cryptix.getEncoding())
  })
})

Client.prototype.getCryptixes = defunction([], '=>[]Cryptix', function getCryptixes() {
  return this.getBlurbs().then((blurbs) => {
    return blurbs.map((blurb) => {
      try {
        return Cryptix.fromEncoding(blurb)
      } catch (err) {
        return undefined
      }
    }).filter((cryptix) => {
